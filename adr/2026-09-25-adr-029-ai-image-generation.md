...
- 날짜: 2026-09-25
- 상태: 제안됨(PR 머지 = 승인)
- 원천: 사용자 결정 2026-09-25(실행 위치 · 한도 초과 · 스타일 · 캡션 · 장 수) · 이슈 #137(스파이크) · #136(사진 자리) · #141(`/blog-write`) · adr-007 · adr-021
...
## 결정 (제안 — 사용자 결정 2026-09-25 반영)

- **(b1)로 간다.** `/blog-write`(#141)는 Claude Code에서 돌리고, **이미지 만들기만** 공식 Codex CLI(`codex exec`)에 맡긴다. 모델은 `gpt-image-2`이고, 비용은 ChatGPT 구독의 Codex 한도 안에서 나간다(추가 과금 0). 서비스(API 서버 · Lambda)는 여전히 어떤 모델도 부르지 않는다. 서비스는 받은 이미지를 adr-021 규칙대로 검사하고 저장만 한다.
  - 호출 모양: `codex exec --sandbox workspace-write -C <작업 폴더> '$imagegen <프롬프트>'`. 한 장씩 부르고, 만들어진 파일을 받아 다음 단계로 넘긴다.
  - 이 기계에는 `codex-cli 0.153.2`가 깔려 있고 ChatGPT 로그인이 되어 있다(2026-09-25).
  - Codex도 MCP 클라이언트라서(`codex mcp`) 흐름 전체를 Codex에서 돌리는 길도 열려 있다. 이번에는 고르지 않았다.
- **한도가 끝나면 사진 자리만 남긴다.** Codex 한도를 다 썼거나 `codex exec`가 실패하면, 그 이미지는 사진 자리(#136)로 남기고 사람이 채운다. API 키 경로(b2)는 두지 않는다. 비용은 늘 0이다.
- **무료 플랜:** ChatGPT Free에도 Codex가 들어 있지만 한도가 작고, `$imagegen`은 한도를 평균 3~5배 빨리 쓴다. 그래서 Free 플랜에서는 **글 하나에 몇 장만** 만들어질 수 있고, 넘는 장은 위 규칙대로 사진 자리로 남는다.
- **글당 장 수:** 기본 4장이다. 사용자가 요청하면 그 글에서 바꾼다.
- **스타일은 글마다 AI가 고른다.** 글 종류를 보고 정한다. 스킬(#141)에 종류별 스타일 안내 표를 두어 제멋대로 흔들리지 않게 한다.

  | 글 종류            | 기본 스타일                        | 피할 것                               |
  | ------------------ | ---------------------------------- | ------------------------------------- |
  | 앱 소개            | 깔끔한 일러스트 · 화면 구성도      | 실제 앱 화면처럼 보이는 가짜 스크린샷 |
  | 육아 이야기 · 일상 | 따뜻한 일러스트(수채 · 손그림)     | 사실 사진풍 아이 얼굴                 |
  | 정보 전달 · 방법   | 도식 · 아이콘 · 사물 중심 일러스트 | 사실과 다른 수치 · 글자가 든 그림     |
  | 장소 · 사물 소개   | 사실 사진풍 가능(인물 없이)        | 실존 장소를 실제 사진처럼 속이는 구도 |

  아이 · 가족 인물은 종류와 상관없이 일러스트로 한다. 사실 사진풍은 딥페이크 표시 · 안전 필터 · 실존 인물 오인 위험이 있다.

- **캡션 "AI로 만든 그림"은 늘 단다.** 법적 의무는 없지만(인공지능기본법은 사업자 의무), 신뢰도와 E-E-A-T 때문이다. 사실 사진풍을 고른 글에서도 이 캡션 덕에 실제 사진으로 오해받지 않는다.
- **흐름:**
  1. AI가 쓴 사진 자리(#136)의 설명 글과 비율을 주문서로 쓴다.
  2. Claude가 영어 프롬프트로 옮기고, 글 종류에 맞는 스타일(위 표)을 붙인다.
  3. 세션이 `codex exec … '$imagegen …'`을 부른다. 실패하거나 한도에 걸리면 그 자리는 사진 자리로 둔다.
  ...
  6. 사진 자리를 이미지 블록으로 바꾼다. 설명 글은 alt가 되고, 캡션에 "AI로 만든 그림"을 붙인다.
- **확인 안 된 것:** `codex exec`에서 `$imagegen`이 도는지는 공식 문서에 명시돼 있지 않다. 커뮤니티 사용 예는 여럿 있다. 첫 구현 이슈에서 로컬로 한 장 만들어 확인한다.
...
- **(b2) API 키 경로(기본이든 예비든):** 한도 걱정이 없고 모델을 고를 수 있다. 하지만 구독과 별도로 요금이 나간다 — **사용자 결정(2026-09-25)으로 뺐다.** 한도가 끝나면 사진 자리로 충분하다.
- **Codex에서 흐름 전체를 돌린다:** 이미지와 글이 한 도구에 있어 매끄럽다. 하지만 글쓰기 · 리서치 · 꾸밈은 지금 Claude Code 흐름에 맞춰 만들고 있고, 사용자가 Claude Code를 골랐다. 길은 열어 둔다(Codex도 MCP 클라이언트).
- **커뮤니티 브리지(로그인 토큰 재사용):** Claude Code 안에서 더 매끄럽다. 하지만 공식 클라이언트 밖에서 ChatGPT 로그인을 쓰는 것이라 약관상 회색이고, 토큰이 새면 계정 전체가 위험하다.
- **(c)만 쓴다(사람이 구독 앱에서 만들어 올린다):** 추가 비용은 0이다. 하지만 "알아서 넣는" 자동화가 안 된다. 한도가 끝났을 때의 사진 자리(#136)로만 남긴다.
- **스타일 하나로 고정(일러스트만):** 블로그 전체 톤은 고르게 된다. 하지만 앱 소개 · 장소 소개처럼 다른 그림이 어울리는 글이 있어 사용자가 "글마다 AI가 고름"을 골랐다.
...
- **Codex 한도를 나눠 쓴다:** 이미지는 같은 한도를 평균 3~5배 빨리 쓴다. 이미지를 많이 만든 날은 Codex로 코딩할 여유가 줄어든다. Free 플랜이면 글당 몇 장만 나온다.
- **한도가 끝나면 사람 손이 든다:** API 키 예비가 없으므로, 넘친 장은 사진 자리로 남고 사람이 채워야 발행할 수 있다.
- **블로그 톤이 고르지 않을 수 있다:** 글마다 스타일을 고르므로 글 목록을 넘겨 볼 때 그림 느낌이 들쭉날쭉할 수 있다. 종류별 스타일 표와 "인물은 늘 일러스트" 규칙으로 흔들림을 줄인다.
- **도구가 둘이 된다:** Claude Code로 쓰려면 Codex CLI 설치 · 로그인이 함께 필요하다. 모델은 `gpt-image-2` 하나로 고정된다.
- **자동화가 로컬 에이전트에 묶인다:** claude.ai 채팅 커넥터로 쓸 때는 AI가 이미지를 만들 수 없고 사진 자리만 남긴다.
- **`codex exec` + `$imagegen` 조합은 공식 문서로 확인되지 않았다:** 동작은 커뮤니티 자료 여러 건에 기댄다. 막히면 사진 자리로 남는다.
...
- **가격 · 한도는 자주 바뀐다:** 표와 한도 설명은 2026-09-25 기준이다.

## 재검토 조건

- 서비스가 외부 사용자에게 열릴 때(2단계): 외부 사용자는 Codex를 쓰지 않을 수 있다.
- OpenAI가 Codex 이미지 생성의 플랜 · 한도 · 약관을 바꿀 때.
- 다른 구독(Google AI Pro · Claude)에 공식 이미지 생성 CLI · 도구가 생길 때.
- 한도 때문에 사진 자리가 자주 남아 사람 손이 부담이 될 때: 그때 API 키 예비를 다시 본다.
- 글마다 고른 스타일이 블로그 톤을 해친다고 느낄 때: 스타일 고정으로 바꾼다.
