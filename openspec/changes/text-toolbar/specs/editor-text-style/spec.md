...
editor-core는 SHALL `setTextStyle`로 건 마지막 글자색 또는 배경색을 에디터 상태에 기억하고, `applyLastColor`가 고른 글자에 그 색을 다시 건다. 기억이 없으면 false다. 단축키는 `Mod-u` 밑줄 · `Mod-Shift-s` 취소선 · `Mod-Shift-h` 마지막 색이다.

#### Scenario: 마지막 색 다시 걸기

- **WHEN** 한 글자에 `setTextStyle({ highlight: "#3366aa" })`를 건 뒤 다른 글자를 고르고 `applyLastColor`를 부른다
- **THEN** 그 글자는 `textStyle { highlight: "#3366aa" }`다

#### Scenario: 기억이 없으면 false

- **WHEN** 색을 건 적 없는 상태에서 `applyLastColor`를 부른다
- **THEN** false다
...
editor-react는 SHALL 도구줄을 선택 시작 글자 위에 틈을 두고 놓고, 기준 틀 위쪽에 자리가 없으면 선택 아래에 놓는다.
