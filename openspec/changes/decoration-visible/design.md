...
- `src`는 `/stickers/{id}.png`다. 에디터의 이미지도 저장 경로(`/images/…`)를 그대로 `src`로 쓴다 — 에디터 앱이 같은 출처에서 서빙한다는 전제가 이미 있다. content-render는 `{imageBaseUrl}/stickers/{id}.png`
...
toDOM은 클립보드 HTML 직렬화에도 쓰인다. 래퍼 규칙(`wrapperRule`)은 내용을 첫 자식(`contentElement`)에서만 읽고, atom은 안쪽을 읽지 않는다 — 스티커 `img`가 붙여넣기에서 이미지 블록이 되지 않는다. 에디터끼리의 복사는 ProseMirror가 slice JSON(`data-pm-slice`)으로 attrs째 옮긴다. HTML에서 스티커를 **읽는** 규칙은 여전히 없다(spec 그대로).

## 4. 스티커 파일 자리

`packages/content-render/assets/stickers/{id}.png` — 스티커 id 목록(content-schema)과 크기 상수(content-render `STICKER_SIZES`)의 짝이 되는 원본이다. 배포(M4 infra)는 이 폴더를 이미지 호스트의 `/stickers/`에 올린다. 사이트 `public/stickers/`에서 바이트 그대로 복사했다(코드 의존 아님). 테스트가 PNG 헤더의 크기와 `STICKER_SIZES`를 대조해 둘이 어긋나지 않게 한다. WebP 변환 · 장당 20KB 게이트는 Lighthouse CI 이슈 몫이다(지금 PNG는 14~27KB).
