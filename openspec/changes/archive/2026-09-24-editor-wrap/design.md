...
- 이 스키마의 목록 항목은 `paragraph (bulletList | orderedList)*`다(content-schema 목록 모양과 같다). 그래서 문단 여럿을 목록으로 감쌀 때 `findWrapping`이 문단 둘을 항목 하나에 넣으려다 실패한다. `wrapRangeInList`가 안 되면 **문단마다 항목 하나**로 감싼다(`replaceWith`). 범위가 모두 문단이고 부모가 그 자리에 목록을 받을 때만 이 경로를 쓴다. `replaceWith`는 교체 구간 안의 선택을 매핑하지 못한다(구간 밖으로 밀려난다). 그래서 k번째 문단 안의 위치를 `원래 + 2 + 2k`로 다시 계산해 선택을 되돌린다(목록 · 항목을 여는 자리 둘, 앞 항목마다 닫고 여는 자리 둘).
...
- `width`는 다루지 않는다. 이미지 · 앱 스크린샷만 가지는 꾸미기인데, 둘 다 인용 · 목록 · 콜아웃 안에 들어갈 수 없어서(스키마 content) 감싸기 범위에 올 일이 없다.
