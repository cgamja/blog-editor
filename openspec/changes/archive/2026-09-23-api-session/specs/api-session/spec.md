...
`POST /api/session`은 SHALL 본문 `{ email, password }`를 계정 테이블(1단계 시드 1행)의 scrypt 해시와 비교하고, 맞으면 204와 HMAC 서명 세션 쿠키를 준다. 쿠키는 `HttpOnly` · `Secure` · `SameSite=Strict` · `Path=/` · `Max-Age`를 갖고, 서명된 값 안에도 만료 시각이 있어 서버가 따로 검사한다. 본문이 JSON이 아니거나 모양이 다르면 400이다.

#### Scenario: 맞는 비밀번호로 로그인하면 속성을 갖춘 세션 쿠키가 온다

- **WHEN** 시드 계정의 email과 비밀번호로 `POST /api/session`을 부른다
...
- **WHEN** 시드 email에 틀린 비밀번호로 로그인한다
- **THEN** 401이고 `Set-Cookie`가 없다

#### Scenario: 없는 계정은 틀린 비밀번호와 같은 응답이다

- **WHEN** 없는 email로 로그인하고, 시드 email에 틀린 비밀번호로 로그인한다
