...
플러그인은 SHALL 열려 있을 때 ArrowUp · ArrowDown · Enter · Tab을 받은 처리기(`onKey`)에 넘기고 그 결과를 돌려준다. `view.composing`이거나 `event.isComposing`이면 넘기지 않고 false다. 닫혀 있으면 어떤 키도 넘기지 않는다.
