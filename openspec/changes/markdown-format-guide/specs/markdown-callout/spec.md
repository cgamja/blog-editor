...
변환은 SHALL 닫는 `:::`가 없는 콜아웃을 여는 줄 번호로 거부하고(`문서 (<m>줄)` 형식), 콜아웃 안의 콜아웃 · 목록/인용 안의 콜아웃 · 정의 밖 `tone` · 내용이 없는 콜아웃을 거부한다.

#### Scenario: 경계 오류는 전부 거부한다

- **WHEN** 닫는 `:::` 없는 `:::callout tone=tip`(3줄째) · 콜아웃 안의 `:::callout` · 목록 항목 안의 `:::callout` · 인용 안의 `:::callout` · `:::callout tone=danger` · `:::callout` 바로 다음 줄 `:::`를 각각 넣는다
- **THEN** 여섯 경우 모두 변환이 실패하고, 첫 경우의 메시지는 `문서 (3줄): 콜아웃이 닫히지 않았다(받음: ":::callout tone=tip") → 끝에 ":::" 줄 추가`다
