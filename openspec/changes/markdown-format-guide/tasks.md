...
- [x] 1.1 `specs/markdown-format/spec.md` — 표준 대응표 · 변형 허용/거부 · 정의 밖 거부 · 가이드 예시 유효성, `specs/markdown-callout/spec.md` — `:::callout` 컨테이너 · 경계 오류, `specs/markdown-directive/spec.md` — `{key=value}` 지시어 · `{frame=app}` · 스티커 문법 없음 → verify: `pnpm exec openspec validate markdown-format-guide --strict`
...
- [x] 2.2 같은 파일 확장 문법 절 — 콜아웃 · 속성 지시어 · 앱 스크린샷 · 스티커 없음 · 스파이크 #2 유보 한 줄 → verify: info 문자열 `example`인 코드 블록의 문자열이 markdown-callout · markdown-directive 시나리오의 WHEN 입력과 같다
