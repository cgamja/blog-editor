...
- [ ] 1.1 스키마 — 시나리오 2개(`doc.test.ts`) → verify: 빨강 원문 보고
- [ ] 1.2 렌더 — 시나리오 1개(`render.test.ts`) → verify: 빨강 원문 보고
- [ ] 1.3 변환 · 직렬화 — 시나리오 3개(`convert.test.ts` · `serialize.test.ts`) → verify: 빨강 원문 보고

## 2. 구현

- [ ] 2.1 `doc.ts` — `NATURAL_SIZE_RANGE` · 짝 검사, 두 arbitrary(content-schema · content-convert)에 크기 → verify: 1.1 초록 · 왕복 property 초록
- [ ] 2.2 `render.ts` — `width` · `height` 속성 → verify: 1.2 초록
- [ ] 2.3 변환 — `size` 키(허용 · 값 검사 · 메시지 · 파서 · 직렬화) + 가이드 표 → verify: 1.3 초록
