# editor-react Specification

## Purpose

React에 TipTap 에디터를 마운트한다 — 초기 문서는 docToNode를 거쳐 마운트 때 한 번만 읽고(다시 만들기는 key로만), 저장은 docFromNode로 한다. 확인용 개발 플레이그라운드를 둔다(adr-019).
...
### Requirement: 조립한 에디터에 정렬 단축키가 있다

`blogEditorExtensions()`는 SHALL editor-core의 `AlignKeys` 확장을 포함한다.

#### Scenario: 조립한 목록에 alignKeys가 있다
