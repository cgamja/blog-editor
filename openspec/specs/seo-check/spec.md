# seo-check Specification

## Purpose

글 내용 SEO를 AI 없이 규칙으로 점검한다 — `checkSeo`(content-schema 순수 함수)가 글 정보 · 문서 · 다른 글 목록을 받아 등급(must · should · info) · 규칙 · 위치 · 고칠 방법을 돌려준다. 저장 · 발행을 막지 않고 MCP 응답과 발행 확인 대화상자가 알린다(이슈 #138, adr-030).
