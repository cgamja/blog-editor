# editor-custom-blocks Specification

## Purpose

커스텀 블록(콜아웃 · 앱 스크린샷)을 넣고 바꾸는 ProseMirror 순수 커맨드와, 그 블록 바로 뒤 Backspace가 블록을 깨지 않게 하는 처리 — 결과 문서는 항상 docFromNode(zod)를 통과한다.
