# editor-table Specification

## Purpose

에디터에서 표를 넣고 편집한다 — 슬래시 메뉴로 넣기, Tab · Shift+Tab 칸 이동, 블록 메뉴의 행 · 열 더하기와 지우기, 열 정렬. 편집 엔진은 `@tiptap/pm/tables`(prosemirror-tables)이고, 모양은 문서 닫힌 집합(머리 행 = 첫 행, 병합 없음, 칸 = 문단 하나)을 벗어나지 않는다(이슈 #130, adr-028).
