...
/** 스티커를 끄는 동안 원래 자리의 스티커를 숨기는 장식(spec: editor-sticker-edit). 등록만 한다 */
const StickerHiding = Extension.create({
  name: "stickerHiding",
  addProseMirrorPlugins: () => [stickerHiding()],
});

/** 그림 폭 손잡이를 끄는 동안의 폭 미리보기 장식(spec: editor-block-resize). 등록만 한다 */
const WidthPreview = Extension.create({
  name: "widthPreview",
 ...
 * 가드 · 되돌리기 · 옮기기 · 정렬 · 키맵 · 슬래시 메뉴 · 입력 규칙 · 목록 키 · 미리 보기 · 스티커 숨김 · 폭 미리보기를 더한다.
 */
