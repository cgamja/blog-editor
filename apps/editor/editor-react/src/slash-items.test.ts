...
describe("editor-slash-menu: 목록 거르기", () => {
  it("WHEN 한글 이름 · 영문 별칭 · 빈 글자 · 없는 말 · table로 거른다 THEN 큰·작은 제목, 큰 제목, 구분선, 전체 11개(끝은 표), 빈 목록, 표", () => {
    ...
    expect(filterSlashItems("")).toHaveLength(11);
    expect(filterSlashItems("").at(-1)).toBe("table");
    expect(filterSlashItems("없는말")).toEqual([]);
    expect(filterSlashItems("table")).toEqual(["table"]);
  });
