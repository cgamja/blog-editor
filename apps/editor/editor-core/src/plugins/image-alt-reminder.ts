 ...
 * 문서는 바꾸지 않고 노드 장식으로 속성만 단다 — 배지 모양은 editor-react CSS가 그린다. 속성 값은 보일 문장이다
 * (CSS `content: attr(...)`) — 문장은 editor-react messages가 가진다.
...
export function imageAltReminder(label = ""): Plugin {
  return new Plugin({
      ...
      decorations(state) {
        ...
        state.doc.descendants((node, pos) => {
          if (node.type.name === "image" && node.attrs.alt === "") {
            decorations.push(
              Decoration.node(pos, pos + node.nodeSize, { [ALT_MISSING_ATTR]: label }),
            );
          }
