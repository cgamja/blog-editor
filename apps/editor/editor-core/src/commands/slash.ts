 ...
 * - closeHistory(되돌리기 묶음 나누기): https://prosemirror.net/docs/ref/#history.closeHistory
 */
import { closeHistory } from "@tiptap/pm/history";
import { EditorState, TextSelection } from "@tiptap/pm/state";
import type { Command } from "@tiptap/pm/state";
import { Transform } from "@tiptap/pm/transform";
import { markSlashMenuClosed, slashMenuKey } from "../plugins/slash-menu";
 ...
 * 이어지는 커맨드는 지운 뒤 상태에서 돌리고 step을 옮겨 담는다(appendCommandSteps — TipTap 체인의 공유 트랜잭션).
 * 되돌리기 묶음은 새로 연다 — 거르기 글자 입력과 합쳐지면 undo 한 번에 `/`까지 사라진다.
...
export function applySlashItem(kind: InsertableBlockKind): Command {
  return (state, dispatch) => {
    ...
    // 체인에서는 state.tr이 공유 트랜잭션이다 — can()만 물을 때는 건드리지 않는다
    ...
    if (dispatch === undefined) return next === null || next(derived);

    const tr = closeHistory(markSlashMenuClosed(state.tr.delete(from, to)));
    tr.setSelection(TextSelection.create(tr.doc, from));
    if (next !== null && !appendCommandSteps(tr, derived, next)) return false;
